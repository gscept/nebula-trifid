...
{
public:
	virtual void doCollideEntity(PhysicsEntity* collidee, Ptr<ContactPoint> contact)=0;
	virtual void doCollideOther(Shape* collidee, Ptr<ContactPoint> contact)=0;
};
...
public:
    ...
    /// set direct shape pointer
    void SetShape(btCollisionShape* c);
    /// get Shape pointer of this physics entity
    btCollisionShape* GetShape() const;
	
	
	RigidBody* GetRigidBody() const;
	RigidBody* GetRigidBodyByName(const Util::String & name) const;

	/// creates a rigid body with a dummy box shape
	/// for fallback use
	void CreateDummyBox(const Math::vector & size, float mass = 0, const Util::String & name = "dummy");
	...
	virtual bool OnCollide(RigidBody* collidee);
	/// called after collision
	virtual void CollideCallback(PhysicsEntity* collidee, Ptr<ContactPoint> contact);
	virtual void CollideCallback(Shape* collidee, Ptr<ContactPoint> contact);
	...
	// HAXX for leveleditor mostly. fugly
	void SetRigidBody(RigidBody *b);
	bool LoadFromResource(bool dontFail = true);
	bool IsLoaded() const;
	bool IsDummy() const;
	void CreateFromGfx(const Util::Array<Ptr<Graphics::ModelEntity>> & models);
...
protected:
    ...
    Ptr<Composite>	body;           // the composite of this entity		
