...
*/
    ...
    this->Verify(n_fequal(v2.length(), 6.244997998f, 0.0001f));
    this->Verify(n_fequal(v2.length3(), 3.741657387f, 0.0001f));
    this->Verify(n_fequal(v2.lengthsq(), 39.0f, 0.0001f));
	...
	this->Verify(float4::permute(v0,v1,0,1,5,6) == float4(1.0f,2.0f,6.0f,7.0f));

	// floor/ceil

	v1.set(1.3, 2.5, 3.8, 5.0);
	v0 = float4::ceiling(v1);
	this->Verify(v0 == float4(2.0f, 3.0f, 4.0f, 5.0f));
	v0 = float4::floor(v1);
	this->Verify(v0 == float4(1.0f, 2.0f, 3.0f, 5.0f));
}
