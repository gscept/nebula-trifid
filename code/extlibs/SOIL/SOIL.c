...
unsigned int SOIL_direct_load_DDS(
		...
		unsigned int* ogl_internal_format,
	...
	)
{
	...
	if( flags & SOIL_FLAG_DDS_LOAD_DIRECT )
	{
		/*	1st try direct loading of the image as a DDS file
			...
			not be flipped, etc.	*/
		tex_id = SOIL_direct_load_DDS( filename, reuse_texture_ID, flags, 0, opengl_texture_type, image_format, num_mips, bpp, data_type );
		if( tex_id )
		{
			/*	hey, it worked!!	*/
			return tex_id;
	...
	tex_id = SOIL_internal_create_OGL_texture(
			...
			GL_MAX_TEXTURE_SIZE, image_format );
	...
	)
{
	...
	if( flags & SOIL_FLAG_DDS_LOAD_DIRECT )
	{
		...
		tex_id = SOIL_direct_load_DDS_from_memory(
				...
				reuse_texture_ID, flags, 1, opengl_texture_type, image_format, num_mips, bpp, data_type );
		if( tex_id )
		{
			/*	hey, it worked!!	*/
			return tex_id;
	...
	)
{
	...
	tex_id = SOIL_internal_create_OGL_texture(
			...
			SOIL_MAX_CUBE_MAP_TEXTURE_SIZE, image_format );
	/*	and nuke the image data	*/
	SOIL_free_image_data( img );
	/*	continue?	*/
	if( tex_id != 0 )
	...
	)
{
	...
	if( flags & SOIL_FLAG_DDS_LOAD_DIRECT )
	{
		/*	1st try direct loading of the image as a DDS file
			...
			DDS file, no MIPmaps will be generated, the image will
			not be flipped, etc.	*/
		tex_id = SOIL_direct_load_DDS( filename, reuse_texture_ID, flags, 1, opengl_texture_type, image_format, num_mips, bpp, data_type );
		if( tex_id )
		{
			/*	hey, it worked!!	*/
			return tex_id;
	...
	tex_id = SOIL_create_OGL_single_cubemap(
			img, width, height, channels,
			face_order, reuse_texture_ID, flags, image_format
			);
		...
		unsigned int* data_type)
{
	/*	variables	*/
	DDS_header header;
	DDS_header_ext extHeader;
	unsigned int isDX10 = 0;
	...
	int mipmaps, cubemap, uncompressed, swapredblue, block_size = 16;
	unsigned int flag;
	unsigned int cf_target, ogl_target_start, ogl_target_end;
	unsigned int ogl_format;
	unsigned int alpha;
	int dx10Fcc = '01XD';
	int dxt1Fcc = '1TXD';
	int dxt2Fcc = '2TXD';
	...
	/* setup default parameters for loading a texture which isn't pixel format described in the DDS */
	(*bpp) = 1;							/* bytes per pixel */
	(*data_type) = GL_UNSIGNED_BYTE;	/* type of the data, default is the standard unsigned byte */
	uncompressed = 1;					/* default is uncompressed, compressed formats must set this to 0 */
	(*ogl_internal_format) = GL_RGBA8;	/* internal format, this can be any sized format according to OpenGL, default is the simple RGBA */
	ogl_format = GL_RGBA;				/* OpenGL format, default is set to RGBA */
	block_size = 4;						/* size of pixel or compressed block, default is integer, so 4 bits */
	swapredblue = 1;					/* swap if texture is float */

	/* check the format of the DDS */
	if ( header.sPixelFormat.dwFlags & DDPF_FOURCC )
	{
		if (header.sPixelFormat.dwFourCC == dx10Fcc)
		{
			/* load extended header */
			memcpy((void*)(&extHeader), (const void*)(&buffer[buffer_index]), sizeof(DDS_header_ext));
			buffer_index += sizeof(DDS_header_ext);

			/* handle sRGB and DXGI (most relevant) formats*/
			if		(extHeader.dwDxgiFormat == 29)				{ (*ogl_internal_format) = GL_SRGB8; }
			else if (extHeader.dwDxgiFormat == 91)				{ (*ogl_internal_format) = GL_SRGB8; ogl_format = GL_BGRA; }
			else if (extHeader.dwDxgiFormat == 93)				{ (*ogl_internal_format) = GL_SRGB8; ogl_format = GL_BGR; }
			else if (extHeader.dwDxgiFormat == 72)				{ (*ogl_internal_format) = GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT1_EXT;	(*bpp) = 1;											block_size = 8; uncompressed = 0; }
			else if (extHeader.dwDxgiFormat == 75)				{ (*ogl_internal_format) = GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT3_EXT;	(*bpp) = 1;											block_size = 16; uncompressed = 0; }
			...
			else if (extHeader.dwDxgiFormat == 32)				{ (*ogl_internal_format) = GL_RGBA8I;								(*bpp) = 1; (*data_type) = GL_BYTE;					block_size = 4;	 } // RGBA8 INT
			else if (extHeader.dwDxgiFormat == 18)				{ (*ogl_internal_format) = GL_RG32I;								(*bpp) = 4; (*data_type) = GL_INT;					block_size = 8; ogl_format = GL_RG; swapredblue = 0; } // RG32 INT
			else if (extHeader.dwDxgiFormat == 34)				{ (*ogl_internal_format) = GL_RG16F;								(*bpp) = 2; (*data_type) = GL_HALF_FLOAT;			block_size = 4; ogl_format = GL_RG; swapredblue = 0; } // RG16 FLOAT
			else if (extHeader.dwDxgiFormat == 36)				{ (*ogl_internal_format) = GL_RG16UI;								(*bpp) = 2; (*data_type) = GL_UNSIGNED_SHORT;		block_size = 4; ogl_format = GL_RG; swapredblue = 0; } // RG16 UINT
			else if (extHeader.dwDxgiFormat == 38)				{ (*ogl_internal_format) = GL_RG16I;								(*bpp) = 2; (*data_type) = GL_SHORT;				block_size = 4; ogl_format = GL_RG; swapredblue = 0; } // RG16 INT
			else if (extHeader.dwDxgiFormat == 50)				{ (*ogl_internal_format) = GL_RG8I;									(*bpp) = 1; (*data_type) = GL_BYTE;					block_size = 2; ogl_format = GL_RG; swapredblue = 0; } // RG8 INT
			else if (extHeader.dwDxgiFormat == 52)				{ (*ogl_internal_format) = GL_RG8UI;								(*bpp) = 1; (*data_type) = GL_UNSIGNED_BYTE;		block_size = 2; ogl_format = GL_RG; swapredblue = 0; } // RG8 UINT
			else if (extHeader.dwDxgiFormat == 41)				{ (*ogl_internal_format) = GL_R32F;									(*bpp) = 4; (*data_type) = GL_FLOAT;				block_size = 4;	ogl_format = GL_RED; swapredblue = 0; } // R32 FLOAT
			else if (extHeader.dwDxgiFormat == 42)				{ (*ogl_internal_format) = GL_R32UI;								(*bpp) = 4; (*data_type) = GL_UNSIGNED_INT;			block_size = 4;	ogl_format = GL_RED; swapredblue = 0; } // R32 UINT
			else if (extHeader.dwDxgiFormat == 43)				{ (*ogl_internal_format) = GL_R32I;									(*bpp) = 4; (*data_type) = GL_INT;					block_size = 4;	ogl_format = GL_RED; swapredblue = 0; } // R32 INT			
			else if (extHeader.dwDxgiFormat == 54)				{ (*ogl_internal_format) = GL_R16F;									(*bpp) = 2; (*data_type) = GL_HALF_FLOAT;			block_size = 2;	ogl_format = GL_RED; swapredblue = 0; } // R16 FLOAT
			else if (extHeader.dwDxgiFormat == 57)				{ (*ogl_internal_format) = GL_R16UI;								(*bpp) = 2; (*data_type) = GL_UNSIGNED_SHORT;		block_size = 2;	ogl_format = GL_RED; swapredblue = 0; } // R16 UINT
			else if (extHeader.dwDxgiFormat == 59)				{ (*ogl_internal_format) = GL_R16I;									(*bpp) = 2; (*data_type) = GL_SHORT;				block_size = 2;	ogl_format = GL_RED; swapredblue = 0; } // R16 INT			
			else if (extHeader.dwDxgiFormat == 57)				{ (*ogl_internal_format) = GL_R8UI;									(*bpp) = 1; (*data_type) = GL_UNSIGNED_BYTE;		block_size = 1;	ogl_format = GL_RED; swapredblue = 0; } // R8 UINT
			else if (extHeader.dwDxgiFormat == 59)				{ (*ogl_internal_format) = GL_R8I;									(*bpp) = 1; (*data_type) = GL_BYTE;					block_size = 1;	ogl_format = GL_RED; swapredblue = 0; } // R8 INT

			// TODO: add the rest of the DX10 formats here...
		}
		else if (header.sPixelFormat.dwFourCC == 36)		{ (*ogl_internal_format) = GL_RGBA16;							(*bpp) = 2; (*data_type) = GL_UNSIGNED_SHORT;	block_size = 8; } // RGBA16 UNORM
		else if (header.sPixelFormat.dwFourCC == 110)		{ (*ogl_internal_format) = GL_RGBA16_SNORM;						(*bpp) = 2; (*data_type) = GL_UNSIGNED_SHORT;	block_size = 8; } // RGBA16 SNORM
		else if (header.sPixelFormat.dwFourCC == 111)		{ (*ogl_internal_format) = GL_R16F;								(*bpp) = 2; (*data_type) = GL_HALF_FLOAT;		block_size = 2; ogl_format = GL_RED;	swapredblue = 0; } // R16 FLOAT
		else if (header.sPixelFormat.dwFourCC == 112)		{ (*ogl_internal_format) = GL_RG16F;							(*bpp) = 2; (*data_type) = GL_HALF_FLOAT;		block_size = 4; ogl_format = GL_RG;		swapredblue = 0; } // RG16 FLOAT
		else if (header.sPixelFormat.dwFourCC == 113)		{ (*ogl_internal_format) = GL_RGBA16F;							(*bpp) = 2; (*data_type) = GL_HALF_FLOAT;		block_size = 8;							swapredblue = 0; } // RGBA16 FLOAT
		else if (header.sPixelFormat.dwFourCC == 114)		{ (*ogl_internal_format) = GL_R32F;								(*bpp) = 4; (*data_type) = GL_FLOAT;			block_size = 4; ogl_format = GL_RED;	swapredblue = 0; } // R32 FLOAT
		else if (header.sPixelFormat.dwFourCC == 115)		{ (*ogl_internal_format) = GL_RG32F;							(*bpp) = 4; (*data_type) = GL_FLOAT;			block_size = 8; ogl_format = GL_RG;		swapredblue = 0; } // RG32 FLOAT
		else if (header.sPixelFormat.dwFourCC == 116)		{ (*ogl_internal_format) = GL_RGBA32F;							(*bpp) = 4; (*data_type) = GL_FLOAT;			block_size = 16;						swapredblue = 0; } // RGBA32 FLOAT
		else if (header.sPixelFormat.dwFourCC == dxt1Fcc)	{ (*ogl_internal_format) = GL_COMPRESSED_RGBA_S3TC_DXT1_EXT;	(*bpp) = 1;										block_size = 8; uncompressed = 0; }
		else if (header.sPixelFormat.dwFourCC == dxt3Fcc)	{ (*ogl_internal_format) = GL_COMPRESSED_RGBA_S3TC_DXT3_EXT;	(*bpp) = 1;										block_size = 16; uncompressed = 0; }
		else if (header.sPixelFormat.dwFourCC == dxt5Fcc)	{ (*ogl_internal_format) = GL_COMPRESSED_RGBA_S3TC_DXT5_EXT;	(*bpp) = 1;										block_size = 16; uncompressed = 0; }
		else
		{
			goto quick_exit;
		}
	...
	{
		/* this is for a DDS without any specific format or compression, assumed to be 8 bit unsigned RGBA, but can also be RGB */
	...
	for( cf_target = ogl_target_start; cf_target <= ogl_target_end; ++cf_target )
	{
		if( buffer_index + DDS_full_size <= buffer_length )
		{
			...
			if( uncompressed )
			{
				/*	and remember, DDS uncompressed uses BGR(A),
					so swap to RGB(A) for ALL MIPmap levels,
					but not for floating point textures */
				if (swapredblue)
				{
					for (i = 0; i < DDS_full_size; i += block_size)
					{
						unsigned int temp;
						memcpy(&temp, &DDS_data[i], *bpp);
						//unsigned char temp = DDS_data[i];
						memcpy(&DDS_data[i], &DDS_data[i + 2 * *bpp], *bpp);
						memcpy(&DDS_data[i + 2 * *bpp], &temp, *bpp);
				...
				glTexImage2D(
					cf_target, 0,
					(*ogl_internal_format), width, height, 0,
					ogl_format, (*data_type), DDS_data);
			} else
			{
				soilGlCompressedTexImage2D(
					cf_target, 0,
			...
			for( i = 1; i <= mipmaps; ++i )
			{
				...
				if( uncompressed )
				{
					...
					glTexImage2D(
						cf_target, i,
						(*ogl_internal_format), w, h, 0,
						ogl_format, (*data_type), &DDS_data[byte_offset]);
				} else
				{
		...
		unsigned int* data_type)
{
	...
	tex_ID = SOIL_direct_load_DDS_from_memory(
		...
		reuse_texture_ID, flags, loading_as_cubemap, opengl_texture_type, image_format, num_mips, bpp, data_type );
...
int query_tex_rectangle_capability( void )
{
	...
	if( has_tex_rectangle_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		if( glewIsExtensionSupported("GL_ARB_texture_rectangle") && 
			...
			glewIsExtensionSupported("GL_NV_texture_rectangle")
			)
		{
			/*	it's there!	*/
...
int query_cubemap_capability( void )
{
	...
	{
		/*	we haven't yet checked for the capability, do so	*/
		if(	glewIsExtensionSupported("GL_ARB_texture_cube_map") &&
