...
// both values must be matched in pixel shader
...
{
__ImplementClass(Environment::AOModule, 'AOMO', Base::EnvironmentModuleBase);
...
AOModule::Setup()
{
	// get horizon based ambient occlusion frame shader
	this->aoFrameShader = FrameServer::Instance()->LookupFrameShader("horizonambientocclusion");

	// gets vital shaders from frame shader
	this->aoShader = this->aoFrameShader->GetFramePassBaseByName("HBAO")->GetShader();
	this->blurShader = this->aoFrameShader->GetFramePassBaseByName("HBAOBlur")->GetShader();

	Ptr<CoreGraphics::RenderTarget> ssaoRenderTarget = this->aoFrameShader->GetRenderTargetByName("SSAOBuffer");

	this->fullWidth = (float)ssaoRenderTarget->GetWidth();
	this->fullHeight = (float)ssaoRenderTarget->GetHeight();
	this->radius = 25.0f;
	this->sceneScale = 0.01f; // should be adjustable

	vars.maxRadiusPixels = 0.1f * min(this->fullWidth, this->fullHeight);
	vars.tanAngleBias = tanf(n_deg2rad(30.0));
	vars.strength = 1.0f;

	// setup basic variables
	this->uvToViewAVar = this->aoShader->GetVariableBySemantic(NEBULA3_SEMANTIC_UVTOVIEWA);
	this->uvToViewBVar = this->aoShader->GetVariableBySemantic(NEBULA3_SEMANTIC_UVTOVIEWB);
	this->rVar = this->aoShader->GetVariableBySemantic(NEBULA3_SEMANTIC_R);
	this->r2Var = this->aoShader->GetVariableBySemantic(NEBULA3_SEMANTIC_R2);
	this->negInvR2Var = this->aoShader->GetVariableBySemantic(NEBULA3_SEMANTIC_NEGINVR2);
	this->focalLengthVar = this->aoShader->GetVariableBySemantic(NEBULA3_SEMANTIC_PIXELFOCALLENGTH);
	this->aoResolutionVar = this->aoShader->GetVariableBySemantic(NEBULA3_SEMANTIC_AORESOLUTION);
	this->invAOResolutionVar = this->aoShader->GetVariableBySemantic(NEBULA3_SEMANTIC_INVAORESOLUTION);
	this->maxRadiusPixelsVar = this->aoShader->GetVariableBySemantic(NEBULA3_SEMANTIC_MAXRADIUSPIXELS);
	this->strengthVar = this->aoShader->GetVariableBySemantic(NEBULA3_SEMANTIC_STRENGHT);
	this->tanAngleBiasVar = this->aoShader->GetVariableBySemantic(NEBULA3_SEMANTIC_TANANGLEBIAS);
	this->powerExponent = this->blurShader->GetVariableBySemantic(NEBULA3_SEMANTIC_POWEREXPONENT);
	this->powerExponent->SetFloat(1.0f);

	this->depthMap = this->aoShader->GetVariableBySemantic("DepthBuffer");
	this->randomMap = this->aoShader->GetVariableBySemantic("RandomMap");

	this->SetupRandomTexture();

	this->randomMap->SetTexture(this->randomTexture);
}
...
{
	// free shaders
	this->aoFrameShader->Discard();
	this->aoFrameShader = 0;
	this->aoShader = 0;
	this->blurShader = 0;
...
*/
void 
AOModule::BeginFrame( const Ptr<Graphics::CameraEntity>& cam )
{
	const Shared::CameraSettings& cameraSettings = cam->GetCameraSettings();
	...
	this->sceneScale = min(this->nearZ, this->farZ);
	vars.r = this->radius * this->sceneScale;
	...
	// set variables for our currently calculated buffer
	...
	// find and assign depth buffer
	if (ResourceManager::Instance()->HasResource("DepthBuffer"))
	{
		Ptr<Texture> depthTexture = ResourceManager::Instance()->LookupResource("DepthBuffer").downcast<Texture>();
		this->depthMap->SetTexture(depthTexture);
	}
}
...
AOModule::SetupRandomTexture()
{
	...
	this->randomTexture = Texture::Create();
	...
	this->randomTexture->SetLoader(loader.upcast<ResourceLoader>());
	this->randomTexture->SetResourceId("RandomMap");
...
*/
void 
AOModule::SetEnabled( bool b )
{
	EnvironmentModuleBase::SetEnabled(b);

	// get frameshader pointer
	this->aoFrameShader->SetEnabled(this->isEnabled);

	// clear if not rendering
	if (!this->isEnabled)
	{
		Ptr<CoreGraphics::RenderTarget> blurredSSAO = this->aoFrameShader->GetRenderTargetByName("SSAOBufferBlurred");
		blurredSSAO->Clear(CoreGraphics::RenderTarget::ClearColor, float4(0,0,0,0), 0, 0);
	}
}
...
*/
void 
AOModule::HandleMessage( const Ptr<Messaging::Message>& msg )
{
	if (msg->CheckId(EnableAmbientOcclusion::Id))
	{
		Ptr<EnableAmbientOcclusion> rMsg = msg.downcast<EnableAmbientOcclusion>();
		this->SetEnabled(rMsg->GetEnabled());
		msg->SetHandled(true);
	}
	else if (msg->CheckId(SetAmbientOcclusionParams::Id))
	{
		Ptr<SetAmbientOcclusionParams> rMsg = msg.downcast<SetAmbientOcclusionParams>();
		vars.maxRadiusPixels = rMsg->GetMaxRadius() * min(this->fullWidth, this->fullHeight);
		vars.tanAngleBias = tanf(n_deg2rad(rMsg->GetAngleBias()));
		vars.strength = rMsg->GetStrength();
		this->sceneScale = rMsg->GetSceneScale();
		this->radius = rMsg->GetRadius();
		this->powerExponent->SetFloat(rMsg->GetPowerExponent());
	}
}
