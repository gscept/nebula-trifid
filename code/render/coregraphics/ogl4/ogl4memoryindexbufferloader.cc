...
OGL4MemoryIndexBufferLoader::OnLoadRequested()
{
	...
	if (this->syncing == IndexBuffer::SyncingSimple)	glBufferData(GL_ELEMENT_ARRAY_BUFFER, this->numIndices * IndexType::SizeOf(this->indexType) * this->bufferCount, this->indexDataPtr, usage | sync);
	else												glBufferStorage(GL_ELEMENT_ARRAY_BUFFER, this->numIndices * IndexType::SizeOf(this->indexType) * this->bufferCount, this->indexDataPtr, GL_MAP_WRITE_BIT | GL_MAP_READ_BIT | sync);
