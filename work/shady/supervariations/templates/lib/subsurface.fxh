	...
	[color0] out vec4 result)
{
	...
	vec4 absorption = textureLod(AbsorptionTexture, screenUv, 0);
	vec4 scatter = textureLod(ScatterTexture, screenUv, 0);
	
	// get color and depth
	vec4 light = DecodeHDR(textureLod(LightTexture, screenUv, 0));
	vec3 color = light.rgb * scatter.rgb;
	
	float depth = textureLod(DepthTexture, screenUv, 0).r;
	...
	for (int i = 0; i < 6; i++)
	{
		vec2 offset = screenUv + o[i] * finalStep * mask.g;
