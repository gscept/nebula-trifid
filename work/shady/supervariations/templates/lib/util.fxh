...
const float MiddleGrey = 1.1f; 
const float MaxLuminance = 16.0f; 
const float MinLuminance = 0.3f;
...
*/
vec4
ToneMap(vec4 vColor, float lumAvg, vec4 luminance)
{	
	...
	return vec4(fLumCompressed * vColor.rgb, vColor.a); 
}
	 ...
	 vec3 specColor)
{
	...
	vec3 fresnelTerm = specColor + ( 1.0f - specColor ) * exponent;
...
{
	ivec2 size = textureSize(tex, 0);	 
	vec2 currentTextureSize = vec2(1.0f, 1.0f) / size;
